...
#### Hence, after checking the stability of both posible values of K; the best choice is 6.

* Implementing the the hirarchical clustering with k=6

```{r}
points_hc_6 <- cutree(hc_fit_wd, k=6)
cereals_clusts_hc_6 <- cbind(points_hc_6, cereals_data)

colnames(cereals_clusts_hc_6)[1] <- "cluster_hc"
head(cereals_clusts_hc_6)
```

```{r , warning=FALSE, fig.width=10, fig.height=8, messages = FALSE}
plot(hc_fit_wd)
rect.hclust(hc_fit_wd, k = 6, border = "blue")
```
...
```{r , warning=FALSE, fig.width=10, fig.height=8, messages = FALSE}
library(cluster)
dist = daisy(x = cereals_clusts_hc_6, metric = "euclidean")
sil_value = silhouette(points_hc_6, dist = dist)
plot(sil_value)
```
...
cereals_clusts_hc_6$cluster_hc <- as.factor(cereals_clusts_hc_6$cluster_hc)

ggparcoord(cereals_clusts_hc_6,
...
* Approaching forward with an elimination technique, we will cancel a few parameters to be condisered while choosing the best cluster for healthy breakfast. 
    - We will consider fat, sodium, sugars and potass for choosing; the cereal having the least nutrition value in these citereans should be eliminated from selection.
